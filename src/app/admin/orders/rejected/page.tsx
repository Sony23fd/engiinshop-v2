...
export default async function RejectedOrdersPage({ searchParams }: { searchParams: Promise<{ days?: string, q?: string, page?: string }> }) {
  ...
  const { groups, totalItems, totalGroups, totalPages = 1 } = await getRejectedOrders(days, page, itemsPerPage, q)
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-slate-500 text-sm mt-1">
            Нийт <strong>{totalItems || 0}</strong> цуцлагдсан барааны захиалга — <strong>{totalGroups || 0}</strong> бүлэг
          </p>
        </div>
